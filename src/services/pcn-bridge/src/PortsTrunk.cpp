...
PortsTrunk::PortsTrunk(Ports &parent, const PortsTrunkJsonObject &conf)
  ...
  if (native_vlan_enabled_)
    ...
    insertAllowed(1);  // set vlan 1 allowed by default
}
...
PortsTrunk::~PortsTrunk() {
  ...
  setNativeVlanEnabled(
      false);  // otherwise delAllowed of the native vlan will fail
  delAllowedList();
}
...
void PortsTrunk::delAllowed(const uint16_t &vlanid) {
  ...
  if (isNative(vlanid)) {
    throw std::runtime_error(
        "Cannot remove the native vlan. Disable the feature or change native "
        "vlan for this port");
  }
...
void PortsTrunk::removeAllowed(const uint16_t vlan) {
  try {
    delAllowed(vlan);
  } catch (std::runtime_error e) {
  }
...
void PortsTrunk::setNativeVlanEnabled(const bool &value) {
  ...
  // manage allowed list
  if (value == true) {
    insertAllowed(native_vlan_);
  } else {
    removeAllowed(native_vlan_);
  }
}
