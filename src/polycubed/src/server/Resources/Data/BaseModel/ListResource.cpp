...
ListResource::ListResource(
    std::function<Response(const std::string &, const ListKeyValues &keys, const nlohmann::json &)>
    ...
    std::function<Response(HelpType, const std::string &, const ListKeyValues &keys)>
        help,
...
//    std::function<Response(const std::string &, const ListKeyValues &keys)>
//        read_whole_handler,
//    std::function<Response(HelpType, const char *name, const ListKeyValues &keys)>
//        help,
//    const std::string &name, const std::string &description,
//    const std::string &cli_example, const std::string &rest_endpoint,
//    const std::string &rest_endpoint_multiple,
...
const Response ListResource::ReadValue(const std::string &cube_name,
  ...
  return read_entry_handler_(cube_name.data(), keys);
}

const Response ListResource::ReadWhole(const std::string &cube_name,
                                       const ListKeyValues &keys) const {
  return read_whole_handler_(cube_name.data(), keys);
}
...
Response ListResource::WriteValue(const std::string &cube_name,
  ...
  case Endpoint::Operation::kCreate:
    return create_entry_handler_(cube_name.data(), keys, value.dump().data());
  case Endpoint::Operation::kReplace:
    return replace_entry_handler_(cube_name.data(), keys, value.dump().data());
  case Endpoint::Operation::kUpdate:
    return update_entry_handler_(cube_name.data(), keys, value.dump().data());
  }
}

Response ListResource::DeleteValue(const std::string &cube_name,
                                   const ListKeyValues &keys) {
  return delete_entry_handler_(cube_name.data(), keys);
}
...
Response ListResource::WriteWhole(const std::string &cube_name,
  ...
  switch (operation) {
  case Endpoint::Operation::kCreate:
    return create_whole_handler_(cube_name.data(), keys, value.dump().data());
  case Endpoint::Operation::kReplace:
    return replace_whole_handler_(cube_name.data(), keys, value.dump().data());
  case Endpoint::Operation::kUpdate:
    return update_whole_handler_(cube_name.data(), keys, value.dump().data());
  }
}

Response ListResource::DeleteWhole(const std::string &cube_name,
                                   const ListKeyValues &keys) {
  return delete_whole_handler_(cube_name.data(), keys);
}

Response ListResource::Help(const std::string &cube_name, HelpType type,
                            const ListKeyValues &keys) {
  return help_(type, cube_name.data(), keys);
}
