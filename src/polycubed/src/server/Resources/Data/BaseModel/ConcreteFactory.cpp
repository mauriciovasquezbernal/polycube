 ...
 * there is unkown resource the function throws an error.
 */
std::unique_ptr<Endpoint::LeafResource> ConcreteFactory::RestLeaf(
  ...
  if (tree_names_.size() == 1) {
    ...
    if (leaf == "type") {
      ...
      // TODO: still not sure if fail or just log a warning
    ...
    if (leaf == "uuid") {
      read_handler_ = [local_core]
        (const std::string &cube_name, const ListKeyValues &keys) -> Response {
        return local_core->base_model()->get_port_uuid(cube_name, keys[0].value);
      };
    } else if (leaf == "status") {
      read_handler_ = [local_core]
        (const std::string &cube_name, const ListKeyValues &keys) -> Response {
        return local_core->base_model()->get_port_status(cube_name, keys[0].value);
      };
    } else if (leaf == "peer") {
      read_handler_ = [local_core]
        (const std::string &cube_name, const ListKeyValues &keys) -> Response {
        return local_core->base_model()->get_port_peer(cube_name, keys[0].value);
      };
