...
void ManagementLib::load() {
  auto factory = std::make_unique<Rest::Resources::Data::Lib::ConcreteFactory>(
      uri_, core_);
  auto base_model_factory = std::make_unique<Rest::Resources::Data::BaseModel::ConcreteFactory>(
      uri_, const_cast<PolycubedCore *>(core_));
  auto parser = Rest::Parser::Yang(std::move(factory), std::move(base_model_factory));
